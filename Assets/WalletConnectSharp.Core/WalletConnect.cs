...
namespace WalletConnectSharp.Core
{
    public class WalletConnect : IDisposable
    {
        ...
        public event EventHandler<WalletConnect> OnOpened;
        public event EventHandler<WalletConnect> OnClose;
        public event EventHandler<WalletConnect> OnError;
        ...
        public async Task Connect()
        {
            ...
            Transport.OpenReceived += OnOpen;
            Transport.CloseReceived += OnCloseReceived;
            Transport.ErrorReceived += OnErrorReceived;

            await Transport.Open(this._bridgeUrl);
        }

        public void OnOpen(object sender, MessageReceivedEventArgs e)
        {
            var walletConnectData = this.Subscribe();

            if (OnOpened != null)
                OnOpened(this, this);
        }

        public async Task<WCSessionData> Subscribe()
        {
        ...
        public void OnCloseReceived(object sender, MessageReceivedEventArgs e)
        {
            if (this.OnClose != null)
                OnClose(this, null);
        }

        public void OnErrorReceived(object sender, MessageReceivedEventArgs e)
        {
        ...
        private async Task<WCSessionData> CreateSession()
        {
            ...
            Events.ListenFor<WCSessionData>("disconnect",
                (sender, @event) => { eventCompleted.TrySetResult(@event.Response); });
