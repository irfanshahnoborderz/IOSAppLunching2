...
namespace WalletConnectSharp.Core.Events
{
    public class EventDelegator : IDisposable
    {
        ...
        public void Trigger(string topic, string json)
        {
            ...
            /*else if (topic != "response:1") // temporary to handle responses of eth requests
            {
                // temporary to handle connected wallet initiated disconnect request
                var providerList = Listeners[Listeners.Keys.ToList()[0]];
                ...
                for (int i = 0; i < providerList.Count; i++)
                {
                    ...
                    provider.PropagateEvent(Listeners.Keys.ToList()[0], json);
                }
            }*/
        }
